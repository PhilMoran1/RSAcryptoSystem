...
class RSACryptoSystem {
    ...
    SHA256(data: string) {
        ...
        let prepro = "";
        let databin = this.stringToBinary(data);
        let databinlen = this.numberToBin(BigInt(databin.length));
        let databinlen64: string = databinlen.length < 64 ?  Array(databinlen.length - 64).fill(0).join('') + databinlen: "0";
        console.log(databinlen64);
        prepro = (databin + (Array(databin.length - 512).fill(0)).join('') + "1" + databinlen64);
